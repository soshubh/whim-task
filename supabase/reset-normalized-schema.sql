-- =============================================================================
-- Whim Task — Reset and recreate the normalized Supabase schema
-- =============================================================================
--
-- WARNING:
--   This script deletes the current Whim Task app tables and recreates them.
--   Run it only if you want a clean reset of the app database.
--
-- Includes:
--   - Drop legacy snapshot/view objects
--   - Recreate normalized tables used by the app
--   - Recreate triggers, RLS policies, grants, storage policies, and realtime
--
-- Safe target objects:
--   profiles
...
-- -----------------------------------------------------------------------------
-- Tear down old Whim Task objects
-- -----------------------------------------------------------------------------
...
drop function if exists public.derive_name_from_email(text) cascade;
drop function if exists public.is_valid_email(text) cascade;
drop function if exists public.is_gmail_address(text) cascade;
drop function if exists public.set_updated_at() cascade;

drop type if exists public.daily_update_slot cascade;
drop type if exists public.notification_sound cascade;
drop type if exists public.reminder_status cascade;
drop type if exists public.reminder_kind cascade;
drop type if exists public.routine_frequency cascade;
drop type if exists public.task_status cascade;
drop type if exists public.task_source cascade;

-- Storage policies live on storage.objects, so remove them explicitly.
...
grant select, insert, update on public.pomodoro_timer_settings to authenticated;
grant select, insert, delete on public.pomodoro_sessions to authenticated;
grant select, insert, delete on public.daily_update_logs to authenticated;
